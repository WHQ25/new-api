...
// 下发给 Vidu 的时长必须与计费估算取自同一个入口。metadata 会被整体反序列化进请求体，
...
func TestConvertToRequestPayloadPinsBilledDuration(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			req: relaycommon.TaskSubmitReq{
				Prompt:   "a cat",
				Metadata: map[string]interface{}{"duration": relaycommon.MaxTaskDurationSeconds + 100},
			},
			want: relaycommon.MaxTaskDurationSeconds,
		},
	}
