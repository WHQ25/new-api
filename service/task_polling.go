...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	// 0. 按次计费 / 提交时已按单位结清的任务不做差额结算
	if bc := task.PrivateData.BillingContext; bc != nil &&
		(bc.PerCallBilling || bc.BillingMode == billing_setting.BillingModeTaskUnitTier) {
		logger.LogInfo(ctx, fmt.Sprintf("任务 %s 按次/单位分档计费，跳过差额结算", task.TaskID))
		return
	}
	// 1. 优先让 adaptor 决定最终额度
	if actualQuota := adaptor.AdjustBillingOnComplete(task, taskResult); actualQuota > 0 {
		RecalculateTaskQuota(ctx, task, actualQuota, "adaptor计费调整", taskSettleAudit{})
		return
	}
	// 2. 回退到 token 重算
	if taskResult.TotalTokens > 0 {
